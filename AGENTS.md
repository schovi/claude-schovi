...
- `plugins/schovi/.codex-plugin/plugin.json` is the Codex plugin manifest.
- `.agents/plugins/marketplace.json` exposes this repository as a Codex marketplace.
- `plugins/schovi/skills/*/SKILL.md` contains Codex-readable skills.
- `plugins/schovi/.claude-plugin/plugin.json` and `.claude-plugin/marketplace.json` keep Claude Code compatibility.
- `plugins/schovi/agents/*/AGENT.md` contains Claude-style subagent instructions. Codex can use them as reference material, but they are not Codex custom agent registrations.

## Codex Usage Notes

- Keep the Codex plugin root at `plugins/schovi/`.
- Add new Codex-facing workflows as skills under `plugins/schovi/skills/<name>/SKILL.md`.
