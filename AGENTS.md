...
| framework-doctor | workflow | `/workflow:framework-doctor` | Validator + cleanup for an initialized repo: deterministic validation (bundled `validate_workflow.py`), refresh of drifted shipped files (status/TEMPLATE) against templates, `tags:` backfill for untagged live tasks, contract sanity plus root-`AGENTS.md` pointer and duplicated-process detection (contract overreach into plugin-owned or session-wide rules, and rival `docs/workflow.md`-style process docs), Codex agent parity (`references/codex-agents.md`). Report first, apply on approval. Not a migrator |
...
Workflow discovery is conservative because the plugin may be installed outside its intended hobby/solo repos. Invoke workflow skills explicitly by default. `work`, `groom`, and `status` may be selected implicitly only when the current repo has `workflow/AGENTS.md` and the request unmistakably refers to that board. `groom` has one further implicit trigger, since it is the board's only entry point: an exploration, audit, or review (yours or an agent's report) that produced concrete work items the user wants tracked. `framework-init` has Codex's `agents/openai.yaml` policy `allow_implicit_invocation: false`, an explicit-only shared description, and a body guard against inherited invocation. It must never be invoked as another skill's missing-framework fallback.
