...
5. **Prefer independent delivery** (nice-to-have, not mandatory): shape the task so it ships on its own. If the ask really splits into parts that could land separately, mint one task per part and let each stand alone — a smaller task that delivers value now beats one big task blocked on everything at once. When a genuine code/data dependency remains (part B literally can't build until part A ships), don't hide it: add a `depends: NNN[, NNN]` line naming the tasks that must reach `done/` first. Skip the line whenever the work is independent — most tasks should have none. `depends:` is task-to-task and can sit in `ready/`; reserve `gate:` + `blocked/` for external facts (an upstream release, an approval).
