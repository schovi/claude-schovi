...
- **Does**: cheap shell that changes or reports process/repo state (`git status`, the validator, `./workflow/status`) — never a shell read of a file (`cat`, `sed -n`, `grep` a path it would then reason about); the rare file the orchestrator legitimately needs goes through `Read`/`Grep` so it deduplicates instead of re-loading every turn. Computes the batch order and dependency plan from that output, dispatches subagents, records each subagent's *condensed* return, writes the report.
...
**A dispatch that comes back without the structured return is a dispatch failure, not a worker failure.** The worker request already demands the return, so the first suspect is the dispatch itself: a spawn mode with no return channel back to the orchestrator. Re-read the adapter, fix the dispatch, and run the unit once more — the adapter says whether its runtime resumes the same worker or re-dispatches, and never open a fresh worker where it doesn't. A second empty return fails the unit (step 3 below); record the raw dispatch result in the report so the cause stays visible. Never reconstruct a unit's outcome from its commits and record that as the return: a commit proves code landed, not that the acceptance gate passed.
