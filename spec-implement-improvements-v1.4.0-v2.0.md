...
modifications: "Validation retry: 2 attempts (not 3)"
---
...
1. **Next Steps Section** (implement.md:830-869): Add explicit `/schovi:publish` reference with usage instructions
2. **Completion Signal** (implement.md:974-982): Execute confetti command instead of just displaying it
3. **Commit Mode Selection** (implement.md:469-515): Add `--verbose` flag for enhanced conventional commits
4. **Spec Parsing** (implement.md:215-296): Support h1/h2 variants, shortened headers, flat task lists
5. **Validation Retry** (implement.md:609-722): Max 2 attempts with status display (modified from 3)
6. **Input Validation** (implement.md:59-63): Clear, actionable error messages for flag conflicts
7. **Progress Updates** (implement.md:412-466): Display activity every 15-20s for tasks >30s
...
- [ ] Update next steps section in `implement.md:830-869` to reference `/schovi:publish` command
...
- [ ] Modify confetti section in `implement.md:974-982` to execute command instead of displaying
  - Change from instruction to execution
  - Add error suppression: `2>/dev/null || true`
  - Add note about non-macOS graceful failure
  - Display completion message after confetti

- [ ] Document commit strategy with `--verbose` flag in `implement.md:469-515`
  - Add Step 2.3 commit mode selection logic
  - Document simplified mode (default): Phase-based format
  - Document enhanced mode (--verbose): Conventional commits with type detection
  - Create comparison table showing differences
  - Add `--verbose` to argument-hint in frontmatter
...
- [ ] Implement robust spec parsing in `implement.md:215-296`
  ...
  - Support patterns: h2 exact, h2 shortened, h1 exact, h1 shortened, singular variant
  - Handle flat task lists (no phase structure) with single-phase fallback
  - Add validation warnings for missing sections
  - Display parsing summary with warnings/successes

- [ ] Add validation retry logic with max 2 attempts in `implement.md:609-722`
  - Add attempt tracking variables (max_attempts = 2, current_attempt)
  - Attempt 1: Initial validation run with status display
  - Attempt 2: Auto-fix if available, or manual fix with targeted changes
  - After 2 attempts: Mark incomplete, document issues, continue
  - Apply pattern to linting, type checking, and tests
  - Display retry status at each attempt: "Attempt 1/2", "Attempt 2/2"
  - Create validation summary showing attempt history

- [ ] Improve input validation error messages in `implement.md:59-63`
  - Add specific error displays for flag conflicts
  - Conflict 1: --output and --no-file (block execution)
  - Conflict 2: --quiet and --no-file (warn, ask confirmation)
  - Conflict 3: --post-to-jira without Jira ID (warn, continue)
  - Conflict 4: --resume without checkpoint (error, suggest options)
...
- [ ] Add progress visibility for long tasks in `implement.md:412-466`
  ...
  - For complex tasks: Display progress every 15-20s
  - Add activity descriptions by task type (file creation, editing, migration, tests)
  - Display task start with timestamp
...
- [ ] Update `schovi/README.md` with v1.4.0 features
  ...
  - Document validation retry behavior
...
- [ ] Create `schovi/agents/verify-executor/AGENT.md` subagent
  ...
  - Implement retry logic within subagent
...
- [ ] Update `implement.md` with subagent execution logic
  ...
  - Update commit creation to use aggregated results
...
- [ ] Implement checkpoint creation logic in `implement.md`
  - Add checkpoint creation after each phase commit
...
- [ ] Implement resume logic with `--resume` flag in `implement.md`
  ...
  - Add Step 6: Delete checkpoint on successful completion
...
- [ ] Add `--dry-run` flag to `implement.md` frontmatter
  - Update argument-hint: [...] [--dry-run]
  - Add flag description in Step 0

- [ ] Implement dry-run detection and banner display
  - Add Step 0.1: Detect --dry-run flag
...
- [ ] Modify all execution steps with dry-run conditionals
  - Before Write/Edit tool execution: Check IF dry_run
...
- **Main Command**: `schovi/commands/implement.md` (primary file being enhanced)
